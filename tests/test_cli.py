...
def test_reports_a_mess_and_exits_one(messy_tree, capsys):
    code, out = run([str(messy_tree), "--no-color", "--backend", "lexical"], capsys)
    assert code == 1
...
def test_tidy_folder_exits_zero(tmp_path, capsys):
    ...
    code, out = run([str(folder), "--no-color", "--backend", "lexical"], capsys)
    assert code == 0
    assert "No mess found" in out


def test_json_output_is_valid_and_complete(messy_tree, capsys):
    code, out = run([str(messy_tree), "--json", "--backend", "lexical"], capsys)
...
def test_fail_over_threshold_is_respected(messy_tree, capsys):
    code, _ = run(
        [str(messy_tree), "--no-color", "--backend", "lexical", "--fail-over", "chaotic"],
        capsys,
    )
    assert code in (0, 1)
    quiet, _ = run(
        [str(messy_tree), "--no-color", "--backend", "lexical", "--fail-over", "tidy"],
        capsys,
    )
    assert quiet == 1
...
def test_missing_folder_is_an_error(tmp_path, capsys):
    assert main([str(tmp_path / "nope"), "--backend", "lexical"]) == 2
...
def test_cli_changes_nothing_on_disk(messy_tree, capsys):
    ...
    run([str(messy_tree), "--no-color", "--backend", "lexical"], capsys)
...
def test_all_flag_shows_tidy_folders(tmp_path, capsys):
    ...
    for i in range(12):
        write_blob(folder / f"DSC_{i:03d}.jpg", 4000 + i)
    _, out = run([str(folder), "--all", "--no-color", "--backend", "lexical"], capsys)
    assert "TIDY" in out
